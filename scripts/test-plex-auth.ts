/**
 * Test script to verify Plex API authentication
 * Run with: npx ts-node scripts/test-plex-auth.ts
 */

async function testPlexAuth(token: string) {
...
if (!token) {
  console.log("Usage: npx ts-node scripts/test-plex-auth.ts <your-plex-token>");
