# PlexGuard Authentication Debugging Guide
...
2. **Check the auth debug log** - The auth route now writes to `/app/data/auth-debug.log`:
   ```bash
   docker exec plexguard cat /app/data/auth-debug.log
   ```
...
- Container logs: `docker logs plexguard`
- Auth debug file: `docker exec plexguard cat /app/data/auth-debug.log`
