...
export default function UsersPage() {
  ...
  const grantBypass = (userId: number, minutes: number) => {
    ...
    // Optimistic UI: show bypass as active immediately
    setBypasses((prev) => ({
      ...prev,
      [userId]: {
        id: -1,
        user_id: userId,
        minutes,
        expires_at: new Date(Date.now() + minutes * 60000).toISOString().replace('T', ' ').substring(0, 19),
        created_by: "admin",
      },
    }));

    // Fire the API call immediately — no await
    ...
    fetch(`/api/users/${userId}/bypass`, {
      ...
      .then(async (res) => {
        ...
        const data = await res.json();
        console.log(`[FRONTEND] Grant bypass response for userId=${userId}:`, data);
        if (data.success) {
      ...
      .catch((error) => {
        clearTimeout(timeoutId);
        console.error(`[FRONTEND] Grant bypass error for userId=${userId}:`, error);
  ...
  const cancelBypass = (userId: number) => {
    console.log(`[FRONTEND] Cancel bypass clicked: userId=${userId} at ${new Date().toISOString()}`);
    
    // Optimistic UI: remove bypass immediately
    setBypasses((prev) => {
      const next = { ...prev };
      delete next[userId];
      return next;
    ...
    fetch(`/api/users/${userId}/bypass`, {
      ...
      .then(async (res) => {
        clearTimeout(timeoutId);
        const data = await res.json();
        console.log(`[FRONTEND] Cancel bypass response for userId=${userId}:`, data);
        if (data.success) {
      ...
      .catch((error) => {
        clearTimeout(timeoutId);
        console.error(`[FRONTEND] Cancel bypass error for userId=${userId}:`, error);
  ...
  const deleteUser = async (userId: number, username: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast({ title: "Deleted", description: `${username} removed from Guardarr` });
        fetchUsers(showDeactivated);
      } else {
